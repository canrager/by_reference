...
def count_entity_positions(dataset: List[Dict[str, Any]], entity_idx: int) -> int:
    """
    Count total number of positions for a given entity across all samples.
    """
    total_positions = 0
    for sample in dataset:
        positions = sample["entity_attribute_token_positions"][f"e{entity_idx}"]
...
def get_activation_cache_for_entity(
    ...
    Args:
        ...
        entity_idx: Index of the entity (1-based)
    ...
    progress_bar = tqdm(total=num_batches, desc=f"Processing entity {entity_idx}")
    ...
    pkl.dump(activation_cache, open("activation_cache.pkl", "wb"))
...
def test_activation_cache(
    ...
    Args:
        ...
        activation_cache: The computed activation cache
        entity_idx: Index of the entity being tested

    Raises:
        AssertionError: If any test fails
    """
    # Test 1: Verify hidden dimension matches model config
    expected_hidden_dim = model.config.hidden_size
    assert (
        activation_cache.shape[1] == expected_hidden_dim
    ), f"Hidden dimension mismatch. Expected {expected_hidden_dim}, got {activation_cache.shape[1]}"

    # Test 2: Verify number of rows matches total entity positions
    expected_rows = count_entity_positions(dataset, entity_idx)
    assert (
        activation_cache.shape[0] == expected_rows
    ), f"Number of rows mismatch for entity {entity_idx}. Expected {expected_rows} positions, got {activation_cache.shape[0]}"

    # Test 3: Verify no NaN or infinite values
    assert not torch.isnan(
        activation_cache
    ).any(), f"Found NaN values in activation cache for entity {entity_idx}"
    assert not torch.isinf(
        activation_cache
    ).any(), f"Found infinite values in activation cache for entity {entity_idx}"

    # Print test results
    print(f"\nTest results for entity {entity_idx}:")
    ...
    position_counts = [
        len(sample["entity_attribute_token_positions"][f"e{entity_idx}"]) for sample in dataset
    ]
    min_positions = min(position_counts)
    max_positions = max(position_counts)
    avg_positions = sum(position_counts) / len(position_counts)

    print(f"\nDataset statistics for entity {entity_idx}:")
...
if __name__ == "__main__":
    ...
    for entity_idx in range(num_entities):
        activation_cache = get_activation_cache_for_entity(model, dataset, entity_idx, layers=layers)
        activation_cache = activation_cache.to(torch.float)

        # Run tests before saving
        test_activation_cache(dataset, model, activation_cache, entity_idx)

        # Save the cache if tests pass
        save_path = f"artifacts/activation_cache_entity{entity_idx}.pt"
